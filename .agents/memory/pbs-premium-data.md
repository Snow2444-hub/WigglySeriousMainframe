...
**How to apply:** Ingest the relationship endpoint for every schedule used in a price-history comparison, attach values by `li_item_id`, and compare those authoritative values to produce premium add/change/remove events. Carry the enclosing schedule code into the upsert because relationship rows may omit it. For historical schedules, use the endpoint's comma-separated `li_item_id` parameter in batches of 25 or fewer: larger lists and long OData `or` filters can return a misleading 404 despite valid records.