            ...
            <div className="flex flex-col md:flex-row justify-between gap-6 md:items-start">
              <div>
                 <div className="mb-3 flex min-w-0 flex-wrap items-center gap-3">
                   <span className="max-w-full break-all rounded-lg bg-primary/10 px-3 py-1 font-mono text-xs font-bold text-primary sm:text-sm">
                    {item.itemCode}
                  </span>
                ...
                <p className="text-lg text-muted-foreground font-medium">
                  {item.drugName} <span className="opacity-40 mx-1.5">•</span> {item.activeIngredient}
                </p>
              ...
              <div className="text-left md:text-right rounded-xl bg-muted/40 p-5 min-w-[200px] border border-border/50">
                ...
                <div className="mt-3 text-xs font-semibold text-muted-foreground flex items-center md:justify-end gap-1.5">
                  <CalendarDays className="h-3.5 w-3.5 text-primary" /> Updated {dateStr(item.lastUpdated)}
                </div>
              </div>
            </div>
          </div>

          <div className="grid lg:grid-cols-[1.5fr_1fr] gap-6 animate-rise-in delay-2">
             <div className="flex min-h-[400px] min-w-0 max-w-full flex-col overflow-hidden rounded-2xl border border-border bg-card shadow-sm">
              <div className="border-b border-border px-6 py-5 flex items-center justify-between">
                <div className="flex items-center gap-2 font-bold text-foreground">
                  <Activity className="h-5 w-5 text-primary" /> Ex-manufacturer / wholesale price history
                </div>
              </div>
            ...
            <div className="flex flex-col gap-6">
              <div className="rounded-2xl border border-border bg-card shadow-sm flex flex-col">
                <div className="border-b border-border px-6 py-5 flex items-center justify-between bg-muted/20">
                  <div className="flex items-center gap-2 font-bold text-foreground">
                    <TrendingDown className="h-5 w-5 text-chart-3" /> Predicted Reductions
                  </div>
                </div>
                <div className="p-0">
                         ...
                         <div key={pred.id} className="p-5">
                           <div className="flex justify-between items-start mb-2">
                             <span className="font-mono text-[10px] font-bold uppercase tracking-wider text-muted-foreground">{dateStr(pred.predictedDate)}</span>
                               <span className="bg-chart-3/15 text-chart-3 font-mono text-[10px] font-bold px-2 py-0.5 rounded uppercase">{pred.subjectToMinisterialDiscretion ? 'Subject to Ministerial discretion' : pred.confidence === 'indicative' ? 'Indicative PBS report' : `${pred.confidence} confidence`}</span>
                           </div>
              ...
              <div className="rounded-2xl border border-border bg-card shadow-sm flex flex-col flex-1">
                <div className="border-b border-border px-6 py-5 flex items-center justify-between bg-muted/20">
                  <div className="flex items-center gap-2 font-bold text-foreground">
                    <History className="h-5 w-5 text-primary" /> Schedule Changes
                  </div>
                </div>
                         ...
                         <div key={change.id} className="p-5 text-sm">
                           <div className="flex items-center justify-between mb-1.5">
                             <span className="font-mono text-[10px] font-bold text-muted-foreground">{dateStr(change.effectiveDate)}</span>
                              <span className={`rounded px-2 py-0.5 font-mono text-[9px] font-bold uppercase ${change.significance === 'high' ? 'bg-destructive/10 text-destructive' : change.significance === 'medium' ? 'bg-accent/20 text-accent-foreground' : 'bg-muted text-muted-foreground'}`}>{change.significance}</span>
                           </div>
